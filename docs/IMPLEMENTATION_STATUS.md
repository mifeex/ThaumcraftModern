# Статус вертикалей после визуального исправления 1.4.24
...
| Research Table / Thaumonomicon | Собственные Menu/Screen, research completion, 7 категорий и 206 definitions; исследования с runtime arcane recipe больше не сохраняют общий импортный `inactive`; 54 общих fallback-иконки заменены item ID результата рецепта | Validator/resource reload/GameTest; 112 действительно незавершённых записей остаются inactive; ручная проверка GUI не проведена |
...
| Thaumcraft Table / Arcane Workbench | Wand conversion с zero-vis, wand slot, vanilla + 113+ shaped/shapeless arcane recipes, research/vis transaction; `CAP_gold`/`CAP_copper` доступны после `BASICTHAUMATURGY` | Forge GameTest проверяет runtime-каталог, availability cap research, conversion, Arcane Stone и vanilla fallback |
